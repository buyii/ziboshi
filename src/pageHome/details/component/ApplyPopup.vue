...
function getDistList(specIndex: number, item: any) {
  const mysku = skusList.value
  const selectItems = selectItem.value.filter(e => e)
  if (selectItems.length > 0) {
    const skuItem = mysku.find((v) => {
      // selectItems  中的值是否都在 item.specIds 中
      return selectItem.value.every((selectItem, index) => {
        if (index !== specIndex) {
          return v.specIds.includes(selectItem) && v.specIds.includes(item.itemId)
        }
        else {
          return true
        }
...
function itemClick(item: any, specIndex: number) {
  ...
  //   return
  // }
  const flag = getDistList(specIndex, item)
  if (flag) {
    return
  }
  ...
  if (selectItems.length === specsList.value.length) {
    ...
    // selectItems  中的值是否都在 item.specIds 中
      return selectItems.every((selectItem) => {
        return v.specIds.includes(selectItem)
      })
    })
  ...
  <view>
    ...
    >
      ...
      <view class="popup-warp" :class="{ 'popup-warp-flag': !myflag, 'popup-pad1': skusList.length === 1 && (!myflag), 'popup-pad2': skusList.length === 2 && (!myflag) }">
        ...
        <view v-for="(spec, specIndex) in specsList" :key="spec.name">
          <view class="skus-title">
            {{ spec.name }}
          </view>
          <view class="skus-warp">
            <view
              v-for="item in spec.specItems" :key="item.itemId" class="skus-item"
              :class="{
                active: (selectItem[specIndex] === item.itemId),
                isDisabled: getDistList(specIndex, item),
                notBgClass: !myflag,
              }"
              @click="itemClick(item, specIndex)"
            >
              <view class="sku-name">
                {{ item.itemName }}
              </view>
      ...
      <!-- <FootButton v-if="myflag" label="确认" :disabled="!selectItem" @confirm="onConfirm" /> -->
