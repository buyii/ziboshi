...
function itemClick(item: any) {
  ...
  if (selectList.value.some((e: any) => e.id === item.id)) {
    selectList.value = selectList.value.filter((e: any) => e.id !== item.id)
  }
  else {
...
function onConfirm() {
  loading.value = true
  emit('confirmAdd', selectList.value)
}
...
<template>
  <root-portal>
    ...
    >
      ...
      <MyScrollView top="116rpx" :fixed="false" :state="state">
        <view class="popup-warp">
          <view v-for="item in kolList" :key="item.id" class="item-box" @click="itemClick(item)">
            ...
            <wd-checkbox v-else :model-value="selectList.some((e: any) => e.id === item.id)" shape="square" size="large" checked-color="#FF0057" />
          </view>
          <view class="btn-box">
      ...
      <FootButton label="确 定" :loading="loading" @confirm="onConfirm" />
    </wd-popup>
  </root-portal>
</template>
