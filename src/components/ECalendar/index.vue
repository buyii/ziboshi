    ...
    dateClick(day) {
      ...
      if (!this.isDisable(this.year, this.month, day)) {
        ...
        if (!this.isRange) {
          ...
          const compare = new Date(date.replace(/-/g, '/')).getTime() < new Date(this.startDate.replace(
            /-/g,
            '/',
          )).getTime()
...
<template>
  <view @touchmove.stop.prevent>
    <view class="l-calendar-box" :class="{ 'calendar-box-show': modelValue }">
      ...
      <view class="date-box">
        <view
          class="date-arrowleft l-icons icon-youjiantou" :style="{ color: yearChangeColor }"
          hover-class="l-opacity" :hover-stay-time="150" @tap="changeYear(false)"
        />
        <view
          class="date-arrowleft l-icons icon-zuojiantou" :style="{ color: monthChangeColor }"
          hover-class="l-opacity" :hover-stay-time="150" @tap="changeMonth(false)"
        />
        <view class="title-datetime">
          {{ showTitle }}
        </view>
        <view
          class="date-arrowright l-icons icon-youjiantou2" :style="{ color: monthChangeColor }"
          hover-class="l-opacity" :hover-stay-time="150" @tap="changeMonth(true)"
        />
        <view
          class="date-arrowright l-icons icon-youjiantou1" :style="{ color: yearChangeColor }"
          hover-class="l-opacity" :hover-stay-time="150" @tap="changeYear(true)"
        />
      </view>
      <view class="date-weekday">
        <view class="date-weekday-item">
          日
        </view>
        <view class="date-weekday-item">
          一
      ...
      <view class="date-content" :style="{ height: `${dateHeight * 6}px` }">
        ...
        <view
          v-for="(item, index) in daysArr" :key="index" class="date-weekday-item"
          :class="{
          ...
          }" :style="{ backgroundColor: getColor(index, 1), height: `${dateHeight}px`, padding: 0 }" @tap="dateClick(index)"
        >
          ...
          <view
            v-if="!lunar && isRange && startDate == `${year}-${month}-${index + 1}` && startDate != endDate" class="date-content-item-desc"
            :style="{ color: activeColor }"
          >
            {{ startText }}
          </view>
          <view
            v-if="!lunar && isRange && endDate == `${year}-${month}-${index + 1}`" class="date-content-item-desc"
            :style="{ color: activeColor }"
          >
            {{ endText }}
          </view>
        </view>
        <view class="bg-mounth">
          {{ month }}
        </view>
      </view>
      ...
      <view class="calendar-text">
        ...
        <view class="calendar-btn">
          <button
            :style="{ opacity: disabled ? '.5' : '1' }" :size="28" :disabled="disabled"
            @click="confireBtnClick(false)"
          >
            确定
          </button>
        </view>
    ...
    <view class="mask" :class="[modelValue ? 'mask-show' : '']" @tap="hide" />
  </view>
</template>
